...
function EmployeesListContent() {
  ...
  const stats = useMemo(() => {
    ...
    /*
      مهم:
      هنجر وكيتا يتم حسابهم
      من مناديب التوصيل النشطين فقط.
    */

    const activeCouriers =
      employees.filter(
        isActiveDeliveryCourier
      );
    ...
    useMemo(() => {
      ...
      return employees.filter(
        (employee) => {
          ...
          /*
            عند فلتر هنجر أو كيتا
            لازم يكون الموظف مندوب توصيل.
          */
...
function PlatformIdBadge({
  ...
  return (
    <span
      className={`inline-flex h-5 items-center gap-1 rounded-md border px-2 text-[9px] font-black leading-none ${badgeStyle}`}
    >
...
function WorkLocationBadge({
  ...
  return (
    <span
      className={`inline-flex rounded-lg border px-2.5 py-1 text-[11px] font-black ${badgeStyle}`}
    >
      {workLocationText(
        value,
        lang
...
function isDeliveryCourier(
  ...
  return deliveryJobs.includes(
    job
  );
}

function isActiveDeliveryCourier(
  employee: Employee
) {
  return (
    normalizeStatus(
